...
Page defects seen on the way, reported not fixed: `/intranet/treasury/` and `/intranet/record/` carry piece addresses in member copy (U-11); `/intranet/record/` overflows a 390px viewport by 31px through `table.state` (U-17); `/commons/` renders em dashes in its lead and body prose ("The cooperative's shared record — named after"), against the house style; the front page's primary action goes to an in-page section rather than the join surface (U-09); the host's 404 page (`/join/`) sets Helvetica.
