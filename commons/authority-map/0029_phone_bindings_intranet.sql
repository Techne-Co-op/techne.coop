-- 0029_phone_bindings_intranet.sql · SMS-04 · the ceremony moves to
-- the intranet, and the binding anchors to the member the record knows
--
-- Not applied. Drafted against issue #247 and the SMS-04 design page at
...
--   human and is decision SMS4-D3 on the design page; it is a data
...
comment on column phone_bindings.agent_id is
  'SMS-04: the member the intranet authenticated, resolved through app_agent_id() at ceremony time. Null on rows minted by the Buzz ceremony before SMS-04, which anchor on member_pubkey alone; naming those is a human act, not a backfill.';
...
comment on column phone_bindings.origin is
  'SMS-04: which ceremony minted the row. buzz is the SMS-03 key-signed ceremony and is the default so existing rows are described truthfully; intranet is the magic-link session ceremony; steward is a binding a named human entered by hand, which carries no possession proof of its own and says so.';

alter table phone_bindings
  add column code_expires_at timestamptz;
comment on column phone_bindings.code_expires_at is
  'SMS-04: when the outstanding one-time code stops being accepted. Set by the sender when the code goes out, cleared on verify. Expiry is enforced in phone_bind_confirm() rather than by a sweep, so a code is dead the moment it is late even if nothing has run.';

alter table phone_bindings
  add column code_attempts smallint not null default 0;
comment on column phone_bindings.code_attempts is
  'SMS-04: failed confirmations against the outstanding code. phone_bind_confirm() refuses at five and retires the row, so a six digit code cannot be walked.';

alter table phone_bindings
  add column code_sent_at timestamptz;
comment on column phone_bindings.code_sent_at is
  'SMS-04: when the challenge SMS was accepted by the carrier. Null while a request waits for the sender to pick it up.';
...
comment on column phone_bindings.status is
  'SMS-04: requested, the member asked and no code exists yet; pending, a code is out and unexpired; verified, the code came back; revoked, a live binding was ended; expired, the code went stale or the attempts ran out; failed, the challenge could not be sent. Only verified admits a number.';
...
comment on column phone_bindings.revoke_reason is
  'SMS-04: why a row left the live set. member_request is the member ending their own binding on the intranet or by STOP; number_reassigned is a steward act when a number demonstrably moved to another person; rebind is the old row retiring as a member binds a different number.';
...
comment on function public.phone_bind_request(text) is
  'SMS-04: a signed-in member asks for a code on a number. Writes the request and nothing else; the database cannot send an SMS, so the sender picks the row up and mints the code. The number never enters the event payload, only the binding row.';
...
comment on function public.phone_bind_confirm(uuid, text) is
  'SMS-04: the member types the code back on the surface they are signed in to. Possession of the number is proven by the code; identity is proven by the session. Five wrong answers retire the row.';
...
comment on function public.phone_bind_revoke(uuid) is
  'SMS-04: the member ends their own binding from the surface that made it. Revocation is a status and a timestamp; the row and its evidence stay, and the member keeps their read of the traffic the binding covered.';
...
-- ============================================================
-- No backfill of agent_id on the two pre-SMS-04 verified rows: there
...
-- steward-only fallback is decision SMS4-D1 and belongs to the
